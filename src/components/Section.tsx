...
export function Tag({ children, tone = 'default' }: { children: ReactNode; tone?: 'default' | 'pop' }) {
  const cls = tone === 'pop' ? 'border-pop/40 text-pop' : 'border-accent-2/40 text-accent-2'
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 font-mono text-[11px] leading-5 ${cls}`}
    >
      {children}
    </span>
  )
